...
# version:   2017-05-18
# Reference: https://github.com/sighsmile/conlleval/blob/master/conlleval_perl.py
...
def countChunks(fileIterator, raw, delimiter, oTag):
    """
...
#     raw = extra_args.raw
#     delimiter = extra_args.delimiter
#     oTag = extra_args.oTag
...
def evaluate(correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter, logger, latex=False):
    # sum counts
    ...
    if not latex:
        ...
        for i in sortedTypes:
            ...
            logger.info("%17s: precision: %6.2f%%; recall: %6.2f%%; FB1: %6.2f   %d" % (i, precision, recall, FB1, foundGuessed[i]))

    # generate LaTeX output for tables like in
    # http://cnts.uia.ac.be/conll2003/ner/example.tex
    else:
        logger.info("        & Precision &  Recall  & F\$_{\\beta=1} \\\\\\hline")
        for i in sortedTypes:
            precision, recall, FB1 = calcMetrics(correctChunk[i], foundGuessed[i], foundCorrect[i])
            logger.info("\n%-7s &  %6.2f\\%% & %6.2f\\%% & %6.2f \\\\" %
...
def evaluate_conll_file(**args):
    ...
    iterator = open(fileName, 'r')
    # process input and count chunks
    correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks(iterator, raw, delimiter, oTag)

    # compute metrics and print
    return evaluate(correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter, logger)
