...
class NNCCorpusReader(XMLCorpusReader):
    """Corpus reader for the XML version of the British National Corpus.
    ...
    You can obtain the full version of the NNC corpus at
    http://www.ota.ox.ac.uk/desc/2554

    If you extracted the archive to a directory called `NNC`, then you can
    instantiate the reader as::

        NNCCorpusReader(root='NNC/Texts/', fileids=r'[A-K]/\w*/\w*\.xml')
    ...
    def _views(self, fileids=None, sent=False, tag=False, strip_space=True, stem=False):
        """A helper function that instantiates NNCWordViews or the list of words/sentences."""
        f = NNCWordView if self._lazy else self._words
...
class NNCWordView(XMLCorpusView):
    """
    A stream backed corpus view specialized for use with the NNC corpus.
    """

    tags_to_ignore = set(
        ['pb', 'gap', 'vocal', 'event', 'unclear', 'shift', 'pause', 'align', 'foreign', 's']
    )
    ...
    def handle_sent(self, elt):
        ...
        return NNCSentence(elt.attrib['n'], sent)